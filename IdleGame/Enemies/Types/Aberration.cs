...
namespace IdleGame.Enemies.Types
{
    public class Aberration : Enemy
    {
        ...
        public static Aberration Generate(int level)
        {
            ...
            name = "Aberration " + Enum.GetName(typeof(SubType), _subType);
            ...
            return new Aberration(name, enemyLevel, reward, health, healthRegen, mana, manaRegen, armour, fireResist, lightningResist, coldResist, damageMin, damageMax, attackSpeed);
        }

        public Aberration(string _name, int lvl, int reward, int hpBase, double hpRegen, int mpBase, double mpRegen, int _armour, double fireRes, double lightningRes, double coldRes, int damageMin, int damageMax, double atkSpeed)
        {
            name = _name;
            type = Type.Beast;
