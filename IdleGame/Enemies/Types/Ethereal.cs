...
namespace IdleGame.Enemies.Types
{
    public class Ethereal : Enemy
    {
        ...
        public static Ethereal Generate(int level)
        {
            ...
            name = "Ethereal " + Enum.GetName(typeof(SubType), _subType);

            reward = 2 * (int)Math.Pow(1.05, enemyLevel);

            health = random.Next(150 * (int)Math.Pow(1.08, enemyLevel), 180 * (int)Math.Pow(1.1, enemyLevel));
            ...
            return new Ethereal(name, enemyLevel, reward, health, healthRegen, mana, manaRegen, armour, fireResist, lightningResist, coldResist, damageMin, damageMax, attackSpeed);
        }

        public Ethereal(string _name, int lvl, int reward, int hpBase, double hpRegen, int mpBase, double mpRegen, int _armour, double fireRes, double lightningRes, double coldRes, int damageMin, int damageMax, double atkSpeed)
        {
            name = _name;
            type = Type.Ethereal;
